...
- **STILL TO COME under Jeopardy (Day 26 / 26b — deferred from the original Day 25 to keep sessions shippable):**
  - **Illness & disease** *(Simon's ask) — SHIPPED Day 28 (v0.28.0):* a plague seeded per world/time spreads along the BOND GRAPH (not live positions), with seeded incubation/course; sick kith go pale/slow/weak, don't court or expedition; recover→immune or die; a 'tending' discovery softens the toll; it burns out and passes. Deterministic + mergeable. Jeopardy trio now complete (predators + catastrophe + plague).
...
- **Day 29 — The larger land map SHIPPED (v0.29.0).** The deferred fractal terrain landed. Root cause of the earlier breakage found and fixed: `climateOf` derived weather from `terrainStats` (the exact coastline), and several tests loop `newWorld()` until non-storm weather — so reshaping terrain shifted how many worlds they discarded and drifted the id sequence. Fixed by **decoupling climate from the terrain shape** (climate now follows realm nature + per-world seed), so weather is stable under any terrain change forever. Then dropped in **multi-octave fractal terrain** (broad continents 3×2, hills 7×4, coves 15×8) + **resolution 160×74** → real, large, contiguous landmasses with organic coastlines. A regression test asserts the land forms continents (a long unbroken run of ground). 312 checks green.
- **Day 27b — Fields (clearing-with-fire) SHIPPED; larger map still deferred.** DONE — **clearing-with-fire → fields**: a seasoned gardener (seed-keeper, restless) sets a careful fire, clears a patch of wild growth, and opens a **field** — ash-rich tilled ground (world content, deterministic id per gardener/day, in `w.structures` type `field`, merges like any structure). Seed-keepers then plant into the nearest field, where the garden grows **richer and faster** (soil ×1.6). Fields render as tilled plots under the plants, and don't count as shelters/beds/village-ring. NOT DONE — a **genuinely larger land map** (fractal continents): I built it (multi-octave valueNoise), but reshaping the identity-derived terrain drifts the global test clock through the suite's conditional-loop timing and cascades into the wanderer/predator-window tests (7 reds). That's test-suite fragility (global-`fakeNow` coupling), not a feature bug — but it needs the suite hardened (deterministic `fakeNow` discipline) FIRST. Deferred until then; the `valueNoise` lattice-size param is landed (backward-compatible) so the change is a small edit once the tests are robust.
...
- **Day 28 — Rituals, myth & biographies; balance & playtest.** RITUALS SHIPPED Day 30 (v0.30.0): funerals for the fallen (elegy drawn from the life; needs bonds + a hearth), naming-days for newborns born into a village, and the evening gathering (with song). Deterministic (keyed to the death/birth/day), mergeable, visible (warm hearth-glow + the folk converge). STILL TO DO from this pillar: **auto-biographies** (every kith a living life-story) and **myth** (the folk's word for the gardener becoming legend).
